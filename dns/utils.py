    ...
    def parse_rr(offset, count):
        ...
        for _ in range(count):
            try:
                ...
                import traceback
                traceback.print_exc()
                # Можно пропустить эту запись и продолжить, или прервать обработку
                # return [], offset # Прервать обработку
        return rrs, offset
...
def build_dns_response(tid, query_data, records, qname, qtype):
    """Собирает DNS-ответ на основе кэша"""
    header = struct.pack("!HHHHHH",
                         tid,  # Transaction ID
                         0x8180,  # Стандартный ответ, recursion available
                         1,  # QDCOUNT
                         len(records),  # ANCOUNT
                         0,  # NSCOUNT
                         0)  # ARCOUNT

    # Секция вопроса (копируем из запроса)
    question_start = 12
